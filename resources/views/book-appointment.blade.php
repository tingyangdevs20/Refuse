...
<body style="font-family: sans-serif;">
                    ...
                    <div class="mainbookappointment">
                        <div class="bookappominetimezone">
                            ...
                            <div class="form-group">
                                ...
                                <input type="hidden" class="appt_date" name="appt_date" value="{{ old('appt_date') }}"
                                    required>
                                <input type="hidden" class="appt_time" name="appt_time" value="{{ old('appt_time') }}"
                                    required>
                        ...
                        <div class="bookappoimetform">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" class="input name" placeholder="Full Name" name="name"
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="tel" class="input mobile" placeholder="Mobile" name="mobile"
                                        id="mobile" value="{{ old('mobile') }}" minlength="11" maxlength="15"
                                        required>
                                    @error('mobile')
                                        <div class="error">{{ $message }}</div>
                                    @enderror
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.reschedule-btn', function() {
                ...
                $(".reschedule_appt_modal").dialog({
                    ...
                    create: function(event, ui) {
                        ...
                        $(this).find('a[data-time="' + previousTime + '"][data-date="' +
                            previousDate + '"]').removeAttr('style');
                        $(this).find('a[data-time="' + previousTime + '"][data-date="' +
                            previousDate + '"]').css({
                            'pointer-events': 'none',
                            'opacity': '0.5'
                        ...
                        $(this).find('a[data-time="' + previousTime + '"][data-date="' +
                            previousDate + '"]').addClass('selected');
                    },
                    beforeClose: function(event, ui) {
                        $('body').css({
                            overflow: 'inherit'
            ...
            // Function to disable slots based on time range
            // Function to disable slots outside the range
            // Function to disable slots outside the range


            function disableSlotsOutsideRange() {
                // Get the current date and time
                var currentDate = new Date();
                currentDate = currentDate.setDate(currentDate.getDate() + advance_booking_duration);
                maximumdate = new Date(currentDate);
                
                // Loop through all slots
                document.querySelectorAll('.myc-available-time').forEach(slot => {
                    var slotTime = slot.getAttribute('data-time');
                    var slotDate = new Date(slot.getAttribute('data-date'));

                    var slotTimeParts = slotTime.split(':');
                    var slotHour = parseInt(slotTimeParts[0]);
                    var slotMinute = parseInt(slotTimeParts[1]);
                    var slotTimeValue = slotHour * 60 +
                    slotMinute; // Convert slot time to minutes since midnight

                    // Calculate the combined time and date value for the slot
                    var slotDateTimeValue = new Date(slotDate);
                    slotDateTimeValue.setHours(slotHour, slotMinute, 0,
                    0); // Set the time for the slot date
                    var slotDate = new Date(slotDateTimeValue.toLocaleString());
                    var slotDatetime = slotDate.getHours() * 60 + slotDate.getMinutes(); // Convert to minutes since midnight
                    // Disable slots outside the range
                    if ( slotDate > maximumdate) {
                        slot.style.pointerEvents = 'none';
                        slot.style.opacity = '0.5'; // You can adjust the opacity or styling as needed
                    }
                });
            }
