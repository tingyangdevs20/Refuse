                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{ $group->campaign_name }} on {{ \Carbon\Carbon::parse($group->pushed_to_camp_date)->format('m/d/Y') }}
                                        @else
                                        {{ $group->pushed_to_camp_date }}
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <button class="btn btn-outline-danger btn-sm" title="Remove {{ $group->name }}"
                                            data-id="{{ $group->id }}" data-toggle="modal" data-target="#deleteModal"><i
            ...
            <form action="{{ route('admin.group.store') }}" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="form-group pt-2">
                        ...
                        <select class="custom-select select2" multiple="multiple" style="width: 100%;" id="tag"
                            name="tag_id">
                    ...
                    <div class="form-group" style="display: none">
                        ...
                        <select class="custom-select" name="email_template" id="email_template">
                            ...
                            <option value="{{ $email_template->id }}" selected>{{ $email_template->template_name }}
                            </option>
                            @endforeach
                            @endif
                        </select>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group pt-2">
                        ...
                        <select class="custom-select" style="width: 100%;" id="modal_market" name="market_id" required>
                            <option value="">Select Market</option>
                            @foreach($markets as $market)
                            <option value="{{ $market->id }}" data-market-name="{{ $market->name }}">{{ $market->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <select class="from-control select2 skip_trace_option" style="width: 100%;" required
                            ...
                            <option value="skip_entire_list_phone"
                                data-amount="${{ @$account->phone_cell_append_rate }}">Skip Trace Phone Numbers (Entire
                                List)  (${{ $account->phone_cell_append_rate }})</option>
                            <option value="skip_records_without_numbers_phone"
                                data-amount="${{ @$account->phone_cell_append_rate }}">Skip Trace Phone Numbers (Records
                            ...
                            <option value="append_emails" data-amount="${{ @$account->name_append_rate }}">Append Email
                                (Records Without Email)(${{ $account->name_append_rate }})</option>
                            <option value="email_verification_entire_list"
                                data-amount="${{ @$account->email_verification_rate }}">Email Verification (Entire List)(${{ $account->email_verification_rate }})
                            </option>
                            <option value="email_verification_non_verified"
                                data-amount="${{ @$account->email_verification_rate }}">Email Verification (Non-Verified
