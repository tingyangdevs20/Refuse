...
class UserAgreementMail extends Command
{
     ...
     * @author Bhavesh Vyas
     */
    protected $signature = 'agreement:mail {contact_id}';
    ...
    public function handle()
    {
        ...
        $contact_id = $this->argument('contact_id');
        ...
        if ($lead->mail_to_owner1) {
            $userAgreementSeller =  UserAgreementSeller::where(["user_id"=> $contact_id, "contact_number"=> "mail_to_owner1"])->first();
            if ($userAgreementSeller) {
                if(!empty($lead->owner1_email1)){
                    $this->sendMailtoUsers($userAgreementSeller, $lead->owner1_email1);
                } else {
        ...
        if ($lead->mail_to_owner2) {
            ...
            if ($userAgreementSeller) {
                if(!empty($lead->owner1_email1)){
                    $this->sendMailtoUsers($userAgreementSeller, $lead->owner2_email1);
                } else {
                    $this->sendMailtoUsers($userAgreementSeller, $lead->owner2_email2);
                }
            }
        }
        if ($lead->mail_to_owner3) {
            $userAgreementSeller =  UserAgreementSeller::where(["user_id"=> $contact_id, "contact_number"=> "mail_to_owner3"])->first();
            if ($userAgreementSeller) {
                if(!empty($lead->owner1_email1)){
                    $this->sendMailtoUsers($userAgreementSeller, $lead->owner3_email1);
                } else {
                    $this->sendMailtoUsers($userAgreementSeller, $lead->owner3_email2);
                }
            }
        }
    ...
    public function sendMailtoUsers($userAgreementSeller, $receiver_email){
        $userAgreement = UserAgreement::where("id", $userAgreementSeller->user_agreement_id)
        ->first();
            $new_array = [];
